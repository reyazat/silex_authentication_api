...
class CompanyUser extends \Illuminate\Database\Eloquent\Model{
	...
	public function addUserCompany($res){
		...
		if(!isset($res['id_company']) || 
		   ...
		   !$this->app['helper']('Utility')->notEmpty($res['maker_identify'])){
			...
			$res['table_visibility'] = '{"deal": "myself", "person": "myself", "company": "myself", "activity": "myself"}';
	...
	public function companyList($idUser){
		...
		$companies = $companyList->toArray();
		foreach($companies as $key=>$company){
			$companies[$key]['company_name'] = $this->app['helper']('Utility')->mysql_escape_mimic_inverse($company['company_name']);
		}
	...
	private function makeFields($details){
		...
		if(isset($details['company_name']) && $this->app['helper']('Utility')->notEmpty($details['company_name'])){
			$fields['company_name'] = $this->app['helper']('Utility')->clearField($details['company_name']);
		}
		...
		if(isset($details['table_visibility']) && $this->app['helper']('Utility')->notEmpty($details['table_visibility'])){
			$fields['table_visibility'] = json_encode($details['table_visibility']);
		}
		...
		if(isset($details['can_assign_to'])){
			$fields['can_assign_to'] = is_array($details['can_assign_to'])?json_encode($details['can_assign_to']):json_encode([]);
		}
		if(isset($details['pipline_order'])){
			$fields['pipline_order'] = is_array($details['pipline_order'])?implode(',',$details['pipline_order']):'';
		} 
