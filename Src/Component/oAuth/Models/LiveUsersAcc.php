...
class LiveUsersAcc extends \Illuminate\Database\Eloquent\Model{
	
	protected $table = 'live_user_acc';
	protected $app;
	...
	public function insertUser($params = []){
		...
		$payLoad = [];
	 	if(!isset($params['id_user']) || 
		    ...
		    !isset($params['company']) || 
			(isset($params['company']) && !$this->app['helper']('Utility')->notEmpty($params['company'])) ||
			!isset($params['url']) || 
			(isset($params['url']) && !$this->app['helper']('Utility')->notEmpty($params['url']))){

				$msg = $this->app['translator']->trans('InvalidParametrs', array('%name%' => 'Owner Company, Company, Id User, Url'));
				...
				LiveUsersAcc::insertGetId($fields);
