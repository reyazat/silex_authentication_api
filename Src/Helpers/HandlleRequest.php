...
class HandlleRequest{
	...
	public function returnResultData($url,$method = 'GET',$data){
		
		$subRequest = Request::create($url,$method,$this->app['helper']('Utility')->notEmpty($data)?$data:[]);
		$response = $this->app->handle($subRequest, HttpKernelInterface::MASTER_REQUEST, false);
		
		return $this->app['helper']('Utility')->convertResponseToArray($response);
